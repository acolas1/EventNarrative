...
def prep_data(config, load=""):
    ...
    train_raw = json.load(open(config['train_file'], 'r', encoding='utf8'))
    ...
    dev_raw = json.load(open(config['dev_file'], 'r', encoding='utf8'))
    test_raw = json.load(open(config['test_file'], 'r', encoding='utf8'))
    if len(load)==0:
...
def eval_g2t(pool, _type, vocab, model, config, display=True):
    ...
    wf_h = open('hyp.txt', 'w', encoding='utf8')
    for i,h in enumerate(hyp):
        wf_h.write(str(h)+'\n')
    wf_h.close()
    wf_r = open('ref.txt', 'w', encoding='utf8')
    ...
    #print(ref)
    with open('file_hyps.txt', 'w',encoding='utf8') as file:
        file.write(json.dumps(hyp))
    #for r in ref:
    #    print(r)
    #    print(type(r))
    ...
    logging.info('BERTscore P,R,F1 {0:}'.format(scorer.score(hyp, ref)))
    return ret[0][-1]
...
def eval_t2g(pool, _type, vocab, model, config, display=True):
    ...
    hyp, ref, pos_label = [], [], []
    model.eval()
    wf = open('t2g_show.txt', 'w', encoding='utf8')
...
def supervise(batch_g2t, model_g2t, optimizerG2T, config, vocab):
...
def train(_type, config, load='tmp_vocab.pt'):
    ...
    model_g2t  = prep_model(config, vocab)
    model_g2t.to(device)
    # model_t2g.to(device)
    ...
    #        for j, (batch_g2t) in enumerate(tqb):
    ...
    #                _loss2, kld = supervise(batch_g2t, model_g2t, optimizerG2T, config, vocab)
    ...
    #            schedulerG2T.step()
    #            klds.append(kld)
    #            tqb.set_postfix({'g2t loss': np.mean(loss_g2t), 'kld loss': np.mean(klds)})
